...
$(() => {
  ...
  const onSelectTheme = function (value) {
  ...
  ReactDOM.render(
    <ul className="theme-picker">
      {themeList.map((item, index) => {
        ...
        return (
          <li
            className="picker-item zoomInRight" key={index} data-key={item} style={{
              animationDelay: `${index / 4}s`,
            }}
          >
            <div className="picker-item-board" style={itemBoardStyle} />
    ...
    <div>
      <Select style={{ width: 300 }} placeholder="搜索组件..." dropdownClassName="component-search-dropdown" onChange={selectChange} optionFilterProp="keyword" optionLabelProp="label">
        {compList.map(item => (
          <Option
            key={item.value}
            keyword={item.value + item.title + item.chinese}
            label={item.chinese}
          >
  ...
  $themePicker.on('click', '.picker-item', (e) => {
